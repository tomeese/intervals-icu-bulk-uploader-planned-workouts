        ...
        <>
          ...
          <details className="text-sm text-slate-600">
            ...
            <ol className="list-decimal ml-5 mt-2 space-y-1">
              <li>Place the file under <code>snapshots/</code> in your repo (keep the filename).</li>
              <li>Commit & push (GitHub Desktop is perfect).</li>
              <li>Run the <b>Weekly Intervals Upload</b> workflow and set
                  <code> plan </code> to <code>{`snapshots/week-${plan!.week_start}.json`}</code>.</li>
            </ol>
          </details>

          <details className="text-xs text-slate-500">
            <summary className="cursor-pointer select-none">Preview JSON</summary>
            <pre className="mt-2 max-h-64 overflow-auto rounded bg-slate-50 p-2 border">{json}</pre>
          </details>
        </>
      ) : (
