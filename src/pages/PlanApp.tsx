/* ./src/pages/PlanApp.txs */ 
/* A React app to generate and submit training plan issues for a GitHub Actions workflow to process. */
...
// Avoid raw 404s in console: probe GitHub Contents API first
async function fetchPlan(pathRel: string) {
  const apiUrl = `https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/contents/${pathRel}?ref=${DEFAULT_BRANCH}`;
  try {
    const head = await fetch(apiUrl, {
      headers: { Accept: "application/vnd.github+json" },
      cache: "no-store",
    });
    if (!head.ok) return null; // file not found or other
    const meta = await head.json();
    const rawUrl = (meta as any)?.download_url as string | undefined;
...
export default function PlanApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setFetching(true);
      const [week, draft, last] = await Promise.all([
        fetchPlan(weeklyPath),
        fetchPlan(draftPath),
        fetchPlan('plans/latest.json'),
      ]);
      if (!live) return;
  ...
  async function loadDraftPreview() {
    ...
    try {
      const apiUrl = `https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/contents/${draftPath}?ref=${DEFAULT_BRANCH}`;
      const head = await fetch(apiUrl, { headers: { Accept: 'application/vnd.github+json' }, cache: 'no-store' });
      if (!head.ok) throw new Error('draft not found');
      const meta = await head.json();
      const dl = (meta as any)?.download_url as string | undefined;
      if (!dl) throw new Error('no download url');
      const res = await fetch(dl, { cache: 'no-store' });
      ...
      // Scroll into view so user sees the result
    ...
    <div className="min-h-screen bg-gradient-to-b from-zinc-50 to-white dark:from-zinc-950 dark:to-zinc-900 text-zinc-900 dark:text-zinc-100">
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <section className="lg:col-span-2 space-y-6">
          ...
          <motion.div ref={previewRef} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22, delay: 0.14 }}
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22, delay: 0.15 }}
            ...
            {draftMode && (
              <div className="mt-3 flex gap-2">
                <button onClick={loadDraftPreview}
                  className="flex-1 px-3 py-2 rounded-xl border border-zinc-300 dark:border-zinc-700 inline-flex items-center justify-center gap-2">
                  <RefreshCw className={`h-4 w-4 ${previewLoading ? 'animate-spin' : ''}`} />
                  Preview Draft (Load/Refresh)
                </button>
              </div>
            )}
            ...
            {draftMode && (
              <div className="mt-4 text-xs text-zinc-600 dark:text-zinc-400">
                <p><strong>Draft flow:</strong> Open Draft Issue → generator commits to <code>{draftPath}</code> → click <em>Preview Draft</em> (or the button above) to load the JSON → when ready, click <em>Promote Draft</em> below.</p>
                <div className="mt-2">
